...
\end{tabular}
\end{table}
بڑی \عددی{r} کی صورت میں \(h_{\ell}^{(1)}(kr)\) جسے ہینکل تفاعک کا پہلا قسم کہتے ہیں \(e^{ikr}/r\) کے لحاظ سے تبدیل ہوتا ہے جبکہ \(h_{\ell}^{(2)}(kr)\) ہینکل تفاعل کی دوسری قسم \(e^{-ikr}/r\) کے لحاظ سے تبدیل ہوگا۔ یوں رخصتی امواج کے لیئے ہمیں کروی ہینکل تفاعلات کی پہلی قسم درکار ہوگی:
\begin{align}
...
\end{align}
اس کا پہلا جز آمدی مستوی موج ہے جبکہ مجموعہ جس کے عددی سر \عددی{C_{l, m}} ہے موج بکھرائو کو ظاہر کرتا ہے۔ چونکہ ہم فرض کر چکے ہیں کہ مخفیہ کروی تشاکلی ہے لحاظہ تفاعل موج \(\phi\) کا تابع نہیں ہوسکتا ہے۔ یوں صرف وہ اجزاء باقی رہیں گے جن میں \(m=0\) ہو یاد رہے \(Y_{\ell}^m\sim e^{im\phi}\) اب مساوات \num{4.27} اور \num{4.32} سے درج ذیل ہوگا
\begin{align}
...
\end{align}
یہ مساوات \num{11.12} میں میں پیش کی گئی عمومی ساخت کے اصول موضوعہ کی تصدیق کرتا ہے اور ہمیں دیکھاتا ہے کہ جزوی موج حیطوں \عددی{a_{\ell}} کی صورت میں حیطہ بکھرائو \(f(\theta)\) کس طرح حاصل ہوگا تفریقی عمودی تراش درج ذیل ہوگا
\begin{align}
...
\end{align*}
یوں بلخصوص \(e^{ikz}\) کو اس طرح بیان کرنا ممکن ہونا چاہیئے اب مبدہ پر \(e^{ikz}\) متناہی ہے لحاظہ نیومن تفاعلات کی اجازت نہیں ہوگی \(r=0\) پر \(n_{\ell}(kr)\) بے قابو بڑھتے ہیں اور چونکہ \(z=r\cos\theta\) میں کوئی \(\phi\) نہیں پایا جاتا ہے لحاظہ صرف \(m=0\) اجزاء ہوں گے۔ مستوی موج کی کروی امواج کی صورت میں سریحاً پھیلائو کلیہ ریلے دیتی ہے۔
\begin{align}
...
\ابتدا{مثال}
کوانٹائی سخت کرہ بکھرائو۔ درج ذیل فرض کریں
\begin{align}
	V(r)=
	\begin{cases}
...
\end{align*}
چونکہ ہم \عددی{ka\ll 1} فرض کر رہے ہیں لحاظہ بلند طاقتیں قابلِ نظرانداز ہوں گی۔ کم توانائی تخمین میں \عددی{\ell=0} جز بکھرائو میں غالب ہوگا۔ یوں کلاسیکی صورت کے لیئے تفریقی عمودی تراش \عددی{\theta} کا تابع نہیں ہوگا۔ ظاہر ہے کہ کم تواانائی سخت کرہ بکھرائو کے لیئے درج ذیل ہوگا 
\begin{align}
...
\انتہا{مثال}
\ابتدا{سوال}
\حوالہء{مساوات \num{11.32}} سے آغاز کرتے ہوئے \حوالہء{مساوات \num{11.33}} ثابت کریں۔ اشارہ: لیژانڈر کثیررکنی کی عمودیت بروئےکار لاتے ہوئے دیکھائیں کہ \عددی{\ell} کی مختلف قیمتوں والے عددی سر لاظماً صفر ہوں گے۔
\انتہا{سوال}
\ابتدا{سوال}
...
\begin{align*}
	V(r) = \alpha\delta(r-a)
\end{align*}
سے کم توانائی بکھراؤ کیصور پر غور کریں جہاں \عددی{\alpha} اور \عددی{a} مستقلات ہیں۔ حیطہ بکھراؤ \عددی{f(\theta)} تفریقی عمودی تراش \عددی{D(\theta)} اور کل عمودی تراش \عددی{\sigma} کا حساب کریں۔ ان میں \عددی{ka\ll1} فرض کریں لحاظہ صرف \عددی{\ell=0} جز خاطرخاہ حصہ ڈالیں گے۔ چیزوں کو آسان بنانے کی خاطر آغاز سے ہی \عددی{\ell\neq0} والے تمام اجزاء کو نظرانداز کریں۔ یہاں \عددی{a_0} تعین کرنا اصل مسئلہ ہے۔ اپنے جواب کو بے بُعدی مقدار \عددی{\beta\equiv2ma\alpha/\hslash^2} کی صورت میں پیش کریں۔
...
\end{align}
چوکور کوسین میں دوسرا جز آمدی کروی موج کو ظاہر کرتا ہے مخفیہ بکھراؤ متعارف کرمے نے یہ تبدیل نہیں ہوگا۔ پہلا جز رخصتی موج ہے جو ینتقل حیط \عددی{\delta_{\ell}} لیتا ہے
\begin{align}
	\psi^{(1)}\approx A\frac{(2\ell+1)}{2ikr}\left[e^{i(kr+2\delta_1)}-(-1)^{\ell}e^{-ikr}\right]P_{\ell}(\cos\theta)&&(V(r)\neq0)
\end{align}
آپ \عددی{e^{ikz}} میں \عددی{h_{\ell}^{(2)}} جز کی بنا پراس کو کروی مرتکز موج تصور کر سکتے ہیں جس میں \عددی{2\delta_{\ell}} ینتقل حیط پایا جاتا ہے اور جو \عددی{e^{ikz}} میں \عددی{h_{\ell}^{(1)}} حصہ کے ساتھ بکھرے موج کی بدولت رخصتی کرویہ موج کے طور پر اُبھرتا ہے۔

\حوالہء{حصہ 11.2.1} میں پورے نظریہ کو جزوی تفاعل حیطوں \عددی{a_{\ell}} کی صورت میں پیش کیا گیا یہاں اس کو ینتقل حیط \عددی{\delta_{\ell}} کی صورت میں پیش کیا گیا۔ ان دونوں کے بیچ ضرور کوئی تعلق پایا جاتا ہوگا۔ یقیناً \حوالہء{مساوات \num{11.23}} کی بڑی \عددی{r} کی صورت میں متقاربی روپ 
\begin{align}
...
\end{align}
اب بھی جزوی موج حیطوں کی بجائے ینتقلات حیط کے ساتھ کام کرنا بہتر ثابت ہوتا ہے چونکہ ان سے طبیعی معلومات باآسانی حاصل ہوتی ہے اور ریاضی کی نقطہ نظر سے ان کے ساتھ کام کرنا آسان ہوتا ہے ینتقلی حیط زاویائی معیارِ حرکت کی بقا کو استعمال کرتے ہوئے مخلوط مقدار \عددی{a_{\ell}} جو دو حقیقی اعدات پر مشتمل ہوتا ہے کی بجائے ایک حقیقی عدد \عددی{\delta_{\ell}} استعمال کرتا ہے۔
%=================
...
\انتہا{سوال}
\ابتدا{سوال}
سخت کرہ بکھراؤ کے لیئے جزوی موج حیطی انتقال \عددی{\delta_{\ell}} کیا ہوں گے \حوالہء{مثال \num{11.3}}؟
\انتہا{سوال}
\ابتدا{سوال}
ایک ڈیلٹا تفاعل خول \حوالہء{سوال \num{11.4}} سے \عددی{S} موج \عددی{\ell=0} جزوی موج انتقال حیط \عددی{\delta_0(k)} تلاش کریں۔ ایسا کرتے ہوئے فرض کریں کہ \عددی{r\to\infty} پر رداسی تفاعل موج \عددی{u(r)} صفر کو پہنچے گا۔
...
\end{align}
ہم با آسانی دیکھا سکتے ہیں کہ یہ \حوالہء{مساوات \num{11.50}} روپ کی مساوات شروڈنگر کو متمعن کرتا ہے
\begin{align*}
...
\begin{align}
	(\nabla^2+k^2)\psi_0 = 0
\end{align}
\حوالہء{مساوات \num{11.67}} مساوات شروڈنگر کی تکملی روپ ہے جو زیادہ معروف تفرقی روپ کی مکمل طور پر معدل ہے۔ پہلی نظر میں ایسا معلوم ہوتا ہے کہ یہ کسی بھی مخفیہ کے لیئے مساوات شروڈنگر کا سری حل ہے جو ماننے والی بات نہیں ہے۔ دھوکہ مت کھائیں۔ دائیں ہاتھ تکمل کی علامت کے اندر \عددی{\psi} پایا جات ہے جسے جاننے بغیر آپ تکمل حاصل کر کے حل نہیں جان سکتے ہیں  تاہم تکملی روپ انتہائی طاقتور ثابت ہوتا ہے اور جیسا ہم اگلے حصہ میں دیکھیں گے یہ بلخصوص بکھراؤ مسائل کے لیئے نہایت موضوع ہے۔
...
\end{align}
جو ٹھیک کلیہ ردرفورڈ \حوالہء{مساوات \num{11.11}} ہے۔ آپ دیکھ سکتے ہیں کہ کولمب مخفیہ کے لیئے کالسیکی میکانیات تخمین بارن اور کوانٹائی نظریہ میدان تمام ایک  جیسا نتیجہ دیتے ہیں۔ ہم کہہ سکتے ہیں کہ کلیہ ردرفورڈ ایک مضبوط کلیہ ہے۔
\انتہا{مثال}
\ابتدا{سوال}
